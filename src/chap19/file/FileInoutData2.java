...
public class FileInoutData2 {
    public static void main(String[] args) {
        ...
        String strLine = null; // 1행 데이터 저장용 변수
        ...
        // 과목 데이터 파일로드
        if (in.open("src/subject_data.csv") == false) {
            System.exit(1);
        }
        // 모든 데이터를 배열로 가져오기
        ...
        // 쓰기 파일 열기
        if (out.open("src/statistics.txt") == false) {
            System.exit(3);
        }

        // 읽기 데이터를 파일에 쓰기
        for (int i = 0; i < subjectList.size(); i++) {
            out.writeIn(subjectList.get(i) + "<===>" + scoreList.get(i));
        }
        // 총점을 계산하고 파일에 쓰기
